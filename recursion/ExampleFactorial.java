package com.company.roman_petrov.recursion;

/* Факториал с помощью рекурсии
Почитать про вычисление факториала.
Реализовать логику метода factorial, где n - это число, факториал которого нужно вычислить.
Не создавайте статические переменные и поля класса.
*/

public class ExampleFactorial {

    public static void main(String[] args) {
        ExampleFactorial example = new ExampleFactorial();

        System.out.println(example.factorial(9));     //362880
        System.out.println(example.factorial(0));     //1
        System.out.println(example.factorial(1));     //1
    }

    public int factorial(int n) {
        if(n <= 1){
            return 1;
        }
        else{
            return n * factorial(n - 1);
        }
    }
}
